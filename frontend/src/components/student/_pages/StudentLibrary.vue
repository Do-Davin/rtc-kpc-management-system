<template>
  <div class="elibrary-wrapper">
    ...
    <EmptyState
      ...
      :message="t('libraryPage.noBooksFound')"
    />
...
const categoryKeys = ['all', 'mathematics', 'physics', 'computerScience', 'informationTechnology'];
const categoryEnglishValues = ['All', 'Mathematics', 'Physics', 'Computer Science', 'Information Technology'];

// Get translated categories based on current language
const translatedCategories = computed(() => {
  return categoryKeys.map(key => t(`libraryPage.${key}`))
})

// Map translated category back to English for filtering
const getEnglishCategory = (translatedCat) => {
  const index = translatedCategories.value.indexOf(translatedCat)
  return index >= 0 ? categoryEnglishValues[index] : 'All'
}
...
const filteredBooks = computed(() => {
  ...
  const englishCategory = getEnglishCategory(selectedCategory.value);
  if (englishCategory && englishCategory !== 'All') {
    filtered = filtered.filter(book => book.category === englishCategory);
  }
