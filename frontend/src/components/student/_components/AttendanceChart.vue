<template>
  <div class="chart-wrapper">
    ...
    <div v-if="error" class="chart-state chart-error">
      <AlertCircle :size="40" />
      <p class="state-title">{{ t('chart.errorTitle') }}</p>
    ...
    <div v-else-if="loading" class="chart-state chart-loading">
      ...
      <p class="state-title">{{ t('chart.loadingTitle') }}</p>
      <p class="state-message">{{ t('chart.loadingMessage') }}</p>
    </div>

    <!-- Empty State -->
    <div v-else-if="!data || data.length === 0" class="chart-state chart-empty">
      <BarChart3 :size="48" />
      <p class="state-title">{{ t('chart.emptyTitle') }}</p>
      <p class="state-message">{{ t('chart.emptyMessage') }}</p>
    </div>
...
const activeSeries = computed(() => {
  ...
  return seriesConfig.value.filter(series => {
...
const getChartPoints = (seriesKey) => {
  ...
  const series = seriesConfig.value.find(s => s.key === seriesKey)
  if (!series) return []
